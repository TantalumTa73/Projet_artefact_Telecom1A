...
def reperage_rotation_prep():
	...
	for i in range(len(info_images)):
		...
		if res is not None:
			orient_img, err = res
			orientations_vect.append(vecteur_2d.rotate_vect(orient_img,-orientations[i]))

	if len(orientations_vect) != 0:
		orientation = vecteur_2d.vect_mean(orientations_vect)
...
def aller_drap():
	...
	print(analyser_drapeau.analyser_drapeau(analyser_drapeau.drapeau_proche(analyse_image.detect_aruco_markers(image, current_pos)), current_pos))
	return render_template("page.html")	
